...
import { ArrowRight, ShieldCheck, Mail, Lock, User, RefreshCw, AlertCircle, CheckCircle2 } from 'lucide-react';
...
export const LoginPage: React.FC = () => {
  ...
  const [step, setStep] = useState<'form' | 'verify_signup' | 'verify_reverify'>('form');
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Monthly 30-day email re-verification required
  ...
  const handleSignUpRequest = async (e?: React.FormEvent, forceResend: boolean = false) => {
    ...
    if (!name.trim() || !email || !email.includes('@') || password.length < 6) {
      setError('Please fill in your name, valid email, and password (at least 6 chars).');
      return;
    }
    ...
    <div className="relative min-h-screen w-screen flex items-center justify-center p-4 overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md glass-card p-8 rounded-3xl shadow-2xl flex flex-col items-center border border-white/15 bg-[#161d2e]/85 backdrop-blur-2xl">
        ...
        <p className="text-xs text-slate-400 mb-5 text-center">
          {mode === 'signin' ? 'Welcome back! Sign in to your workspace' : 'Create your account to access AI workspace'}
        </p>
        ...
        {infoMsg && (step === 'verify_signup' || step === 'verify_reverify') && (
